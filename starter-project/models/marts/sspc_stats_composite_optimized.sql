/*
  Optimized Composite SSPC Stats Model with Intelligent Fallback

  OPTIMIZATION IMPROVEMENTS:
  1. Direct partition filtering (_PARTITIONDATE) for efficient partition pruning
  2. Early column pruning - only load necessary columns for quality calculation
  3. Simplified source selection logic using COALESCE
  4. Eliminated redundant UNION ALL + deduplication
  5. Reduced data movement with late column expansion
  6. More efficient anti-join pattern using EXCEPT DISTINCT
...
with quality_metrics as (
    ...
    where _PARTITIONDATE between '2025-10-10' and '2025-10-20'  -- Direct partition filter
    group by 1, 2, 3
),
...
-- Step 3: Unified source selection (simplified logic)
source_selection as (
...
data_30sec_filtered as (
    ...
    inner join source_selection ss
        ...
        and ss.selected_source = '30sec'  -- Filter early
    where d._PARTITIONDATE between '2025-10-10' and '2025-10-20'
),

-- Step 5: Get 300sec data only when needed (quality insufficient or no 30sec data)
data_300sec_filtered as (
    ...
    inner join source_selection ss
        ...
        and d.sspc_id = ss.sspc_obj_id
        and cast(d.terminal_id as int64) = ss.terminal_obj_id
        and ss.selected_source = '300sec'  -- Filter early
    where d._PARTITIONDATE between '2025-10-10' and '2025-10-20'
)
